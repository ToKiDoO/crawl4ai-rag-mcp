        ...
        try:
            ...
            if result.get("success") and "results" in result:
                ...
                if "summary" in result:
                    ...
                    print(f"- URLs processed: {summary.get('urls_processed', 'unknown')}")
                    print(f"- Total content length: {summary.get('total_content_length', 'unknown')}")
                    print(f"- Processing time: {summary.get('processing_time_seconds', 'unknown')}s")
                ...
                if success_count == len(test_urls):
                    ...
                    print(f"✅ Array parameter handling works correctly")
                    print(f"✅ Concurrent processing completed successfully")
                    return True
                else:
...
async def test_single_vs_batch_comparison():
    ...
    async with MCPClient(command) as client:
        ...
        single_response = await client.call_tool("scrape_urls", {
        ...
        try:
            ...
            if single_success and 'content_length' in single_result:
                ...
                single_content_length = single_result['summary'].get('total_content_length', 0)
                
            print(f"Single URL result: Success={single_success}, Content={single_content_length} chars, Time={single_elapsed:.2f}s")
        except Exception as e:
        ...
        batch_response = await client.call_tool("scrape_urls", {
        ...
        try:
            ...
            if batch_success and 'summary' in batch_result:
                batch_content_length = batch_result['summary'].get('total_content_length', 0)
                
            print(f"Batch URL result: Success={batch_success}, Content={batch_content_length} chars, Time={batch_elapsed:.2f}s")
        except Exception as e:
        ...
        if single_success and batch_success:
            print(f"- Content consistency: {'✅ CONSISTENT' if abs(single_content_length - batch_content_length) < 100 else '⚠️ DIFFERENT'}")
            print(f"- Performance: Single={single_elapsed:.2f}s, Batch={batch_elapsed:.2f}s")
...
async def test_error_handling():
    ...
    test_cases = [
        {
            "name": "Invalid URLs",
            "urls": ["https://invalid-domain-that-does-not-exist-12345.com", "https://another-invalid-domain-67890.com"],
            "expected": "partial_failure"  # Should handle gracefully
        },
        {
            "name": "Mixed valid/invalid URLs",
            "urls": ["https://example.com", "https://invalid-domain-that-does-not-exist-12345.com"],
    ...
    async with MCPClient(command) as client:
        ...
        for test_case in test_cases:
            ...
            try:
                response = await client.call_tool("scrape_urls", {
                    "url": test_case["urls"],
                    "return_raw_markdown": True,
                    "max_concurrent": 1
                })
                ...
                if result.get("success"):
                    if "results" in result:
                        results = result["results"]
                        success_count = sum(1 for content in results.values() if content != "No content retrieved")
                        total_count = len(results)
                        
                        print(f"  Result: {success_count}/{total_count} URLs succeeded")
                        
                        if test_case["expected"] == "partial_success" and success_count > 0:
                            print("  ✅ Partial success handling works correctly")
                        elif test_case["expected"] == "partial_failure" and success_count < total_count:
...
if __name__ == "__main__":
    async def run_comprehensive_tests():
        ...
        print("=" * 70)
        print(f"1. Batch URL scraping functionality: {'✅ PASS' if batch_test_passed else '❌ FAIL'}")
        print(f"2. Single vs batch comparison: {'✅ PASS' if comparison_test_passed else '❌ FAIL'}")
        print(f"3. Error handling: {'✅ PASS' if error_test_passed else '❌ FAIL'}")
        
        overall_success = batch_test_passed and comparison_test_passed and error_test_passed
