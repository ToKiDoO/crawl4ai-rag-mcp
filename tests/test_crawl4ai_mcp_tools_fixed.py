...
class TestMCPToolsFixed:
    ...
    async def test_search_tool_success(self, mock_context, mcp_server):
        ...
        with patch("services.search_and_process", new_callable=AsyncMock) as mock_search_process:
    ...
    async def test_scrape_urls_tool_success(self, mock_context, mcp_server):
        ...
        with patch("services.process_urls_for_mcp", new_callable=AsyncMock) as mock_process_urls:
    ...
    async def test_smart_crawl_url_success(self, mock_context, mcp_server):
        ...
        with patch("services.smart_crawl.smart_crawl_url", new_callable=AsyncMock) as mock_smart_crawl:
    ...
    async def test_get_available_sources_success(self, mock_context, mcp_server):
        ...
        with patch("src.tools.get_available_sources_wrapper", new_callable=AsyncMock) as mock_get_sources:
    ...
    async def test_perform_rag_query_success(self, mock_context, mcp_server):
        ...
        with patch("src.tools.perform_rag_query_wrapper", new_callable=AsyncMock) as mock_rag_query:
    ...
    async def test_search_code_examples_success(self, mock_context, mcp_server):
        ...
        with patch("src.tools.search_code_examples_wrapper", new_callable=AsyncMock) as mock_search_code:
    ...
    @pytest.mark.asyncio
    async def test_check_ai_script_hallucinations_success(self, mock_context, mcp_server):
        """Test AI script hallucination checking successfully."""
        # Get the check_ai_script_hallucinations tool from registered tools
        check_ai_script_hallucinations = mcp_server._tools["check_ai_script_hallucinations"]
        ...
        with patch("utils.validation.validate_script_path") as mock_validate:
            with patch("src.tools.get_app_context") as mock_get_context:
                ...
                with patch("knowledge_graph.validation.check_ai_script_hallucinations", new_callable=AsyncMock) as mock_check:
                    ...
                    result = await check_ai_script_hallucinations(
                        mock_context, "/path/to/valid_script.py"
                    )
                    result_data = json.loads(result)
    ...
    @pytest.mark.asyncio
    async def test_check_ai_script_hallucinations_enhanced_success(self, mock_context, mcp_server):
        ...
        check_ai_script_hallucinations_enhanced = mcp_server._tools["check_ai_script_hallucinations_enhanced"]
        ...
        with patch("utils.validation.validate_script_path") as mock_validate:
            with patch("src.tools.get_app_context") as mock_get_context:
                ...
                with patch("knowledge_graph.enhanced_validation.check_ai_script_hallucinations_enhanced", new_callable=AsyncMock) as mock_enhanced_check:
    ...
    async def test_query_knowledge_graph_success(self, mock_context, mcp_server):
        ...
        with patch("src.tools.query_knowledge_graph_wrapper", new_callable=AsyncMock) as mock_kg_query:
    ...
    async def test_parse_github_repository_success(self, mock_context, mcp_server):
        ...
        with patch("utils.validation.validate_github_url") as mock_validate_url:
            with patch("src.tools.parse_github_repository_wrapper", new_callable=AsyncMock) as mock_parse_repo:
                ...
                result = await parse_github_repository(
                    mock_context, "https://github.com/test/repo.git"
                )
                result_data = json.loads(result)
