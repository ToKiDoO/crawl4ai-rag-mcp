...
async def test_mcp_tool_call(tool_name: str, arguments: Dict[str, Any]) -> Dict[str, Any]:
...
async def test_batch_url_scraping():
    ...
    try:
        ...
        response = await test_mcp_tool_call("scrape_urls", {
            "url": test_urls,
        ...
        if "result" in response:
            ...
            if result.get("success") and "results" in result:
                ...
                successful_urls = [url for url, content in results.items() if content != "No content retrieved"]
