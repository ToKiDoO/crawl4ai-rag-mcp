...
class TestMCPProtocol:
    ...
    def test_tool_discovery(self):
        ...
        for tool_name, tool_obj in tools.items():
            assert hasattr(tool_obj, 'fn'), f"Tool '{tool_name}' should have fn attribute"
            assert hasattr(tool_obj, 'name'), f"Tool '{tool_name}' should have name attribute"
            assert hasattr(tool_obj, 'description'), f"Tool '{tool_name}' should have description attribute"
            assert hasattr(tool_obj, 'parameters'), f"Tool '{tool_name}' should have parameters attribute"
    ...
    def test_parameter_validation_types(self):
        ...
        for tool_name, param_name, expected_type in test_cases:
            ...
            if isinstance(expected_type, list):
                ...
                assert 'oneOf' in param_schema or 'anyOf' in param_schema or 'type' in param_schema
            else:
    ...
    def test_tool_descriptions(self):
        ...
        for tool_name, tool_obj in tools.items():
            ...
            assert not desc.startswith('TODO'), f"Tool '{tool_name}' has placeholder description"
        ...
        for tool_name, tool_obj in tools.items():
            ...
            for param_name, param_schema in properties.items():
                if 'enum' in param_schema:
                    ...
                    assert isinstance(enum_values, list), f"{tool_name}.{param_name} enum should be list"
                    assert len(enum_values) > 0, f"{tool_name}.{param_name} enum should not be empty"
    ...
    def test_array_parameters(self):
        ...
        assert 'oneOf' in url_schema or 'anyOf' in url_schema or url_schema.get('type') == 'array'
...
class TestMCPEdgeCases:
    ...
    async def test_empty_array_parameter(self):
        ...
        assert "empty" in response["error"].lower() or "url" in response["error"].lower()
