...
''')
    ...
    result = await call_mcp_tool("check_ai_script_hallucinations", {
    ...
    if isinstance(result, dict):
        if result.get("success") is False:
            ...
            if "Knowledge graph functionality not available" in error:
                print("⚠️  Knowledge graph not configured - this is expected without Neo4j")
                print(f"   Error: {error}")
...
async def test_enhanced_detection():
    ...
    result = await call_mcp_tool("check_ai_script_hallucinations_enhanced", {
        "script_path": test_script,
...
async def main():
    ...
    results.append(("check_ai_script_hallucinations", await test_hallucination_detection()))
