...
class TestFileMetrics:
    ...
    def test_file_sizes(self):
        ...
        print("\n=== File Size Analysis ===")
        for file_path, lines in sorted(file_sizes.items(), key=lambda x: x[1], reverse=True):
            status = "✅" if lines <= 400 else "⚠️"
            print(f"{status} {str(file_path):40} {lines:4} lines")
...
class TestModularityBenefits:
    ...
    def test_selective_imports(self):
        ...
        for module in list(sys.modules.keys()):
            if module.startswith(('core', 'config', 'utils', 'database', 'services', 'knowledge_graph')):
                del sys.modules[module]
        ...
        assert utils_memory < 20, f"Selective import uses too much memory: {utils_memory} MB"
