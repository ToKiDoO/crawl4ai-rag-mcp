...
async def test_throughput_10_urls_concurrent():
    ...
    # Assertions
    assert successful == len(TEST_URLS), f"Expected all {len(TEST_URLS)} to succeed, but only {successful} did"
    # With 100ms per URL and concurrent execution, should complete in ~0.2-0.5 seconds
    assert duration < 2.0, f"Expected to complete in under 2 seconds, but took {duration:.2f} seconds"
    
    # Performance metrics
    throughput = len(TEST_URLS) / duration
    assert throughput > 5, f"Expected throughput > 5 URLs/second, but got {throughput:.2f}"
