...
"""
        ...
        if code_blocks_small:
            for i, block in enumerate(code_blocks_small):
                print(f"   Block {i+1}: {len(block['code'])} chars, language: {block.get('language', 'unknown')}")
        ...
        print(f"   Found {fixes_found} occurrences of 'extract_code_blocks(md, min_length=100)'")
        
        if fixes_found >= 2:  # Should be in both _process_multiple_urls and smart_crawl_url
