...
class TestMCPToolWrappers:
    ...
    @pytest.mark.asyncio
    async def test_perform_rag_query_wrapper_with_source_filter(self, mock_database_factory):
    ...
    @pytest.mark.asyncio
    async def test_perform_rag_query_wrapper_error_handling(self, mock_database_factory):
        """Test RAG query wrapper error handling."""
        # Mock database error
        mock_database_factory.search_crawled_pages.side_effect = Exception("Database connection failed")
    ...
    @pytest.mark.asyncio
    async def test_search_code_examples_wrapper_with_validated_search(self, mock_validated_search_service, mock_database_factory):
                ...
                "results": [
                    {
                        ...
                        "validation": {
                            ...
                            "validation_checks": [
                                {"check": "method_exists", "passed": True, "weight": 0.7}
                            ],
    ...
    @pytest.mark.asyncio
    async def test_search_code_examples_wrapper_neo4j_unavailable(self, mock_database_factory):
        ...
        with patch.dict(os.environ, {}, clear=True):
            ...
            result_data = json.loads(result)
            assert "validation_summary" not in result_data  # No validation when Neo4j unavailable
            assert len(result_data["results"]) == 1

    @pytest.mark.asyncio
    async def test_search_code_examples_wrapper_validation_error(self, mock_validated_search_service):
        ...
        with patch.dict(os.environ, {
            ...
            mock_validated_search_service.search_and_validate_code.side_effect = Exception("Neo4j connection failed")
...
class TestWrapperIntegration:
    ...
    async def test_end_to_end_code_search_flow(self, integration_environment):
        ...
        with patch('tools.get_database_adapter') as mock_db, \
    ...
    async def test_wrapper_fallback_mechanism(self, integration_environment):
        ...
        with patch('tools.get_database_adapter') as mock_db, \
            ...
            result_data = json.loads(result)
            assert len(result_data["results"]) == 1
            assert "validation_summary" not in result_data  # No validation in fallback mode
