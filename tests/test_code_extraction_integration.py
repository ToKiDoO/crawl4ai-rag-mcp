...
async def test_code_extraction_integration():
    ...
    try:
        ...
        if result_data.get("success"):
            ...
            print(f"   Code examples stored: {result_data.get('code_examples_stored', 0)}")
            ...
            if code_examples_count > 0:
                ...
                try:
                    ...
                    if code_search_result:
                        print(f"✅ Found {len(code_search_result)} code examples in database")
                        # Show first example
                        if code_search_result:
                            first_example = code_search_result[0]
                            print(f"   Sample code: {first_example.get('code_example', '')[:100]}...")
                    else:
...
"""
        ...
        if code_blocks_small:
            for i, block in enumerate(code_blocks_small):
                print(f"   Block {i+1}: {len(block['code'])} chars, lang: {block.get('language', 'unknown')}")
...
if __name__ == "__main__":
    async def run_tests():
        ...
        try:
            import subprocess
            result = subprocess.run(['docker', 'ps', '--filter', 'name=qdrant-dev', '--format', '{{.Names}}'], 
        ...
        if overall_success:
            ...
            if test2_success and 'qdrant-dev' in subprocess.run(['docker', 'ps'], capture_output=True, text=True).stdout:
