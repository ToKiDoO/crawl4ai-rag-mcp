...
async def test_mcp_scrape_urls():
    ...
    async with aiohttp.ClientSession() as session:
        ...
        try:
            async with session.post(mcp_url, json=valid_urls_payload, headers=headers) as resp:
                result = await resp.json()
                print(f"Valid URLs test result:")
                if 'result' in result and 'content' in result['result']:
                    ...
                    if content.get('success'):
                        ...
                        print(f"❌ Processing failed: {content.get('error', 'unknown error')}")
                else:
        ...
        try:
            async with session.post(mcp_url, json=invalid_urls_payload, headers=headers) as resp:
                result = await resp.json()
                print(f"Invalid URLs test result:")
                if 'error' in result:
                    ...
                    if not content.get('success'):
                        ...
                        print("❌ Invalid URLs were NOT rejected - this indicates a problem!")
                else:
                    print(f"❌ Unexpected response format: {result}")
