...
class TestNeo4jCodeExtractor:
    ...
    @pytest.mark.asyncio
    async def test_extract_repository_code_repository_not_found(self, code_extractor, mock_neo4j_session):
    ...
    @pytest.mark.asyncio
    async def test_extract_repository_code_success(self, code_extractor, mock_neo4j_session):
    ...
    @pytest.mark.asyncio
    async def test_extract_classes_with_methods(self, code_extractor, mock_neo4j_session):
    ...
    @pytest.mark.asyncio
    async def test_extract_functions_filtering(self, code_extractor, mock_neo4j_session):
...
class TestExtractRepositoryCodeFunction:
    ...
    async def test_extract_repository_code_success(self, mock_repo_extractor):
        ...
        with patch('knowledge_graph.code_extractor.Neo4jCodeExtractor') as mock_extractor_class:
    ...
    async def test_extract_repository_code_error(self, mock_repo_extractor):
        ...
        with patch('knowledge_graph.code_extractor.Neo4jCodeExtractor') as mock_extractor_class:
            mock_instance = AsyncMock()
            mock_extractor_class.return_value = mock_instance
            mock_instance.extract_repository_code.side_effect = Exception("Extraction failed")
