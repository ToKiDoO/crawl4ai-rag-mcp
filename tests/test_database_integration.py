...
"""],
    ...
    @pytest.mark.asyncio
    async def test_add_documents_to_database_success(self, sample_crawl_data, mock_openai_embeddings):
        ...
        await utils_refactored.add_documents_to_database(
            mock_adapter,
    ...
    async def test_search_documents_integration(self, mock_openai_embeddings):
        ...
        results = await utils_refactored.search_documents(
            mock_adapter,
            query_embedding,
            match_count=5
        )
    ...
    @pytest.mark.asyncio
    async def test_code_examples_integration(self, sample_code_examples, mock_openai_embeddings):
        ...
        await utils_refactored.add_code_examples_to_database(
            mock_adapter,
    ...
    async def test_concurrent_database_operations(self, mock_openai_embeddings):
        ...
        tasks = []
        for i in range(5):
            task = utils_refactored.add_documents_to_database(
                mock_adapter,
    ...
    async def test_hybrid_search_integration(self, mock_openai_embeddings):
        ...
        query_embedding = [0.1] * 1536
        vector_results_actual = await utils_refactored.search_documents(
            mock_adapter,
            query_embedding,
            match_count=5
        )
    ...
    async def test_large_embedding_batches(self, mock_openai_embeddings):
        ...
        contents = [f"Content for page {i}" * 100 for i in range(large_batch_size)]  # Large content
        metadatas = [{"page": i, "size": "large"} for i in range(large_batch_size)]
        url_to_full_document = {url: f"Full content {i}" * 200 for i, url in enumerate(urls)}
    ...
    async def test_search_with_filters(self, mock_openai_embeddings):
        ...
        query_embedding = [0.1] * 1536
        await utils_refactored.search_documents(
            mock_adapter,
            query_embedding,
            match_count=5,
    ...
    async def test_get_documents_by_url(self):
        ...
        assert results == mock_results
        mock_adapter.get_documents_by_url.assert_called_once_with("https://example.com/page1")
    ...
    async def test_get_available_sources(self):
        ...
        mock_sources = [
            ...
            {"source_id": "docs.example.com", "summary": "Documentation", "total_word_count": 5000}
        ]
