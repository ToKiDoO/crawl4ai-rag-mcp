...
class TestQdrantErrorHandling:
    ...
    async def test_invalid_embedding_dimensions(self):
        ...
        with patch('database.qdrant_adapter.QdrantClient') as mock_client_class:
            ...
            mock_client.upsert.side_effect = Exception("Vector dimension mismatch: expected 1536, got 128")
            mock_client_class.return_value = mock_client
    ...
    async def test_search_with_invalid_embedding(self):
        ...
        with patch('database.qdrant_adapter.QdrantClient') as mock_client_class:
            mock_client = Mock()
            mock_client.get_collection.return_value = Mock()
            mock_client.search.side_effect = ValueError("Invalid query vector dimensions")
            mock_client_class.return_value = mock_client
            ...
            with patch('utils_refactored.create_embedding', return_value=[0.1] * 128):  # Wrong dimension
            ...
            with patch('utils_refactored.create_embeddings_batch', return_value=[[0.1] * 1536] * 10):
                ...
                with pytest.raises(Exception, match="Second batch failed"):
                    await add_documents_to_database(
                        ...
                        url_to_full_document={f"https://test.com/doc{i}": f"full {i}" for i in range(10)},
                        batch_size=5
                    )
    ...
    async def test_api_key_authentication_failure(self):
        ...
        with patch('database.qdrant_adapter.QdrantClient') as mock_client_class:
            mock_client = Mock()
            mock_client.get_collection.side_effect = Exception("Authentication failed: Invalid API key")
            mock_client_class.return_value = mock_client
    ...
    async def test_collection_does_not_exist_during_operation(self):
        ...
        with patch('database.qdrant_adapter.QdrantClient') as mock_client_class:
            ...
            mock_client.upsert.side_effect = Exception("Collection 'crawled_pages' does not exist")
            mock_client_class.return_value = mock_client
    ...
    async def test_factory_error_handling(self):
        ...
        with patch('database.qdrant_adapter.QdrantAdapter.initialize', side_effect=Exception("Init failed")):
