...
def test_import_fix():
    ...
    print(f"✅ Database parameter usage in search_code_examples (should be 2): {database_param_in_func}")
    
    # Assertions
    assert old_import_count == 0, f"Found {old_import_count} occurrences of old import"
    assert new_import_count == 2, f"Expected 2 occurrences of new import, found {new_import_count}"
    assert async_call_count == 2, f"Expected 2 async calls, found {async_call_count}"
    assert database_param_in_func == 2, f"Expected 2 database parameter usages in search_code_examples, found {database_param_in_func}"
...
def test_utils_refactored_implementation():
    ...
    if 'async def search_code_examples(' in content:
        ...
        raise AssertionError("❌ Missing async search_code_examples in utils_refactored")
...
def main():
    ...
    try:
        ...
        print("- Updated function calls to use 'database' parameter instead of 'client'")
