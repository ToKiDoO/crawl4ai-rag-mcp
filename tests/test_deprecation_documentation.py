...
def analyze_warnings(warning_lines: List[str]) -> Tuple[Set[str], Set[str]]:
    ...
    for line in warning_lines:
        ...
        if not matched and ('DeprecationWarning' in line or 'PendingDeprecationWarning' in line):
...
def generate_report(found_warnings: Set[str], undocumented_warnings: Set[str], 
                   missing_from_tests: List[str]) -> bool:
...
def main():
    ...
    # Generate report
    is_complete = generate_report(found_warnings, undocumented_warnings, missing_from_tests)
