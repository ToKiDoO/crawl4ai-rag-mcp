...
from tools import (
    parse_github_repository_wrapper,
    get_app_context,
    set_app_context
)
...
from knowledge_graph.enhanced_validation import EnhancedHallucinationDetector, check_ai_script_hallucinations_enhanced
...
class TestMCPToolContextWrappers:
    ...
    async def test_extract_and_index_repository_code_workflow_simulation(
        ...
        with patch('utils.create_embeddings_batch') as mock_embeddings:
            ...
            embedding_texts = [ex["embedding_text"] for ex in extraction_result["code_examples"]]
            ...
            await mock_app_context.database_client.delete_repository_code_examples("test-repo")
            await mock_app_context.database_client.add_code_examples(
                urls=[f"neo4j://repository/test-repo/{ex['code_type']}/{ex['name']}" for ex in extraction_result['code_examples']],
                chunk_numbers=list(range(len(extraction_result['code_examples']))),
                code_examples=[ex["code_text"] for ex in extraction_result['code_examples']],
                summaries=[f"{ex['code_type'].title()}: {ex['full_name']}" for ex in extraction_result['code_examples']],
                metadatas=[ex["metadata"] for ex in extraction_result['code_examples']],
                embeddings=embeddings,
                source_ids=[ex["repository_name"] for ex in extraction_result['code_examples']]
            )
            
            # Verify workflow steps were executed
            mock_embeddings.assert_called_once_with(embedding_texts)
            mock_app_context.database_client.delete_repository_code_examples.assert_called_once_with("test-repo")
            ...
            assert all("neo4j://repository/test-repo" in url for url in call_args["urls"])
            assert all("test-repo" in source_id for source_id in call_args["source_ids"])
    ...
    async def test_extract_and_index_repository_code_extraction_failure(
        ...
        with patch('knowledge_graph.code_extractor.extract_repository_code') as mock_extract:
            ...
            result = await mock_extract(mock_app_context.repo_extractor, "nonexistent-repo")
            assert result["success"] is False
    
    @pytest.mark.asyncio
    async def test_smart_code_search_workflow(self, mock_app_context, mock_fastmcp_context):
        ...
        with patch('services.validated_search.ValidatedCodeSearchService') as mock_service_class:
            ...
            service = ValidatedCodeSearchService(
                ...
                neo4j_driver=mock_app_context.repo_extractor.driver if mock_app_context.repo_extractor else None
            )
    ...
    async def test_smart_code_search_performance_requirement(
        ...
        fast_results = {
            "success": True,
            "results": [{"content": "fast result", "similarity": 0.8, "validation": {"confidence_score": 0.7}}],
        ...
        with patch('services.validated_search.ValidatedCodeSearchService') as mock_service_class:
            ...
            assert execution_time < 2.0, f"Search took {execution_time:.2f}s, expected <2.0s"
            assert result["success"] is True
...
class TestCodeExtractionWorkflow:
    ...
    def mock_neo4j_session(self):
        ...
        async def mock_run(query, **params):
            if "Repository" in query and "name: $repo_name" in query:
                return repo_result
            elif "Class" in query and "collect" in query:
                result = AsyncMock()
    ...
    async def test_extract_repository_code_success(self, mock_neo4j_session):
        ...
        mock_repo_extractor.driver.session.return_value.__aenter__.return_value = mock_neo4j_session
        mock_repo_extractor.driver.session.return_value.__aexit__ = AsyncMock(return_value=None)
        ...
        total_items = summary.get("classes", 0) + summary.get("methods", 0) + summary.get("functions", 0)
        assert total_items >= 0
    
    @pytest.mark.asyncio
    async def test_extract_repository_code_repository_not_found(self, mock_neo4j_session):
    ...
    async def test_code_indexing_with_embeddings(self, mock_code_examples):
        ...
        with patch('utils.create_embeddings_batch') as mock_embeddings:
            ...
            for i, example in enumerate(mock_code_examples):
                urls.append(f"neo4j://repository/auth-service/{example['code_type']}/{example['name']}")
            ...
            assert all("auth-service" in source_id for source_id in call_args["source_ids"])
    
    @pytest.mark.asyncio
    async def test_code_indexing_embedding_dimension_validation(self, mock_code_examples):
...
class TestValidatedSearchFunctionality:
    ...
    @pytest.mark.asyncio
    async def test_validated_search_with_high_confidence(self, mock_validated_search_service):
...
'''
    ...
    async def test_enhanced_hallucination_detection_success(
        self, mock_enhanced_detector, mock_python_script
    ):
        ...
        with patch('pathlib.Path.exists', return_value=True), \
            ...
            assert any("super_authenticate" in suggestion for suggestion in result["suggestions"])
    ...
    async def test_enhanced_hallucination_detection_function_integration(self):
        ...
        from knowledge_graph.enhanced_validation import check_ai_script_hallucinations_enhanced
    ...
    @pytest.mark.asyncio
    async def test_enhanced_hallucination_detection_performance(self, mock_enhanced_detector):
        ...
        assert execution_time < 5.0, f"Detection took {execution_time:.2f}s, expected <5.0s"
        assert result["success"] is True
...
class TestIntegrationPerformanceBenchmarks:
    ...
    async def test_end_to_end_workflow_performance(self):
        ...
        with patch('knowledge_graph.code_extractor.extract_repository_code') as mock_extract:
            ...
            with patch('utils.create_embeddings_batch') as mock_embeddings:
                ...
                extraction_result = await extract_repository_code(mock_app_context.repo_extractor, "test-repo")
                
                end_time = time.time()
                extraction_time = end_time - start_time
                
                # Performance target for extraction and indexing
                assert extraction_time < 3.0, f"Extraction took {extraction_time:.2f}s, expected <3.0s"
                assert extraction_result["success"] is True
    ...
    async def test_concurrent_operations_performance(self):
        ...
        with patch('services.validated_search.ValidatedCodeSearchService') as mock_service_class:
            ...
            assert throughput > 5.0, f"Throughput was {throughput:.1f} ops/s, expected >5.0 ops/s"
