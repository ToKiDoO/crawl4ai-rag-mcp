...
def test_simple_deprecation_capture():
    ...
    try:
        ...
        found_pydantic = "PydanticDeprecatedSince20" in output or "class-based `config` is deprecated" in output
        found_fake_http = "read_text is deprecated" in output or "fake_http_header" in output
        
        print("\n📊 Results:")
        print(f"✅ Pydantic deprecation warning: {'Found' if found_pydantic else 'Not found'}")
        print(f"✅ fake_http_header warning: {'Found' if found_fake_http else 'Not found'}")
...
def verify_documentation_exists():
    ...
    has_pydantic = "PydanticDeprecatedSince20" in content or "pydantic" in content.lower()
