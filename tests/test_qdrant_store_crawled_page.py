...
class TestQdrantStoreCrawledPage:
        ...
        class EnhancedFakeQdrantClient(FakeQdrantClient):
            ...
            def search(
                ...
                self.search_calls.append(
                    (collection_name, len(query_vector), limit, query_filter),
                )
                if self.should_fail:
        ...
        with patch(
            ...
            await add_documents_to_database(
                ...
                url_to_full_document={
                    "https://example.com/page1": "Full document content",
                },
                batch_size=10,
            )
    ...
    async def test_store_crawled_page_multiple_chunks(
        ...
        with patch(
            ...
            await add_documents_to_database(
                ...
                url_to_full_document={
                    "https://example.com/long-page": "Complete page content",
                },
                batch_size=10,
            )
    ...
    async def test_store_crawled_page_batch_processing(
        ...
        for i in range(5):
            ...
            url_to_full_document[f"https://example.com/page{i}"] = (
                f"Full content for page {i}"
            )
    ...
    async def test_store_crawled_page_with_source_ids(
        ...
        guide_doc = next(
            doc for doc in stored_docs if "guide" in doc["payload"]["content"]
        )
    ...
    async def test_store_crawled_page_empty_data(
        ...
        with patch("utils.create_embeddings_batch", return_value=[]):
            await add_documents_to_database(
                database=adapter,
                urls=[],
                chunk_numbers=[],
    ...
    async def test_store_crawled_page_metadata_handling(
        ...
        doc1 = next(
            doc for doc in stored_docs if "Content 1" in doc["payload"]["content"]
        )
        doc2 = next(
            doc for doc in stored_docs if "Content 2" in doc["payload"]["content"]
        )
        doc3 = next(
            doc for doc in stored_docs if "Content 3" in doc["payload"]["content"]
        )
