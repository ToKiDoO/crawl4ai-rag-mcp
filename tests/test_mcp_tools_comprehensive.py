...
class TestMCPToolsComprehensive:
    ...
    async def test_search_successful_scraping(self, mock_context):
        ...
        with patch.dict(os.environ, {'SEARXNG_URL': 'http://localhost:8080'}):
            ...
            with patch('aiohttp.ClientSession', return_value=mock_session):
                ...
                with patch('src.utils_refactored.add_documents_to_database', new_callable=AsyncMock):
    ...
    async def test_scrape_urls_success(self, mock_context):
        ...
        with patch('src.crawl4ai_mcp.add_documents_to_database', new_callable=AsyncMock):
    ...
    async def test_perform_rag_query_success(self, mock_context):
        ...
        with patch('src.utils_refactored.search_documents', new_callable=AsyncMock) as mock_search:
    ...
    async def test_search_code_examples_enabled(self, mock_context):
        ...
        with patch.dict(os.environ, {'USE_AGENTIC_RAG': 'true'}):
            ...
            with patch('src.utils.search_code_examples', return_value=mock_results) as mock_search:
    ...
    async def test_check_ai_script_hallucinations_enabled(self, mock_context):
        ...
        with patch.dict(os.environ, {'USE_KNOWLEDGE_GRAPH': 'true', 'NEO4J_URI': 'bolt://localhost:7687'}):
            test_script = '''
            import requests
            def fetch_data():
            ...
            with patch('src.crawl4ai_mcp.validate_script_path', return_value={"valid": True}) as mock_validate:
    ...
    async def test_get_available_sources_success(self, mock_context):
        ...
        mock_adapter.get_sources.return_value = [
            ...
            {"source_id": "docs.python.org", "summary": "Python docs", "total_word_count": 15000}
        ]
