...
def test_environment_variable_fix():
    ...
    assert old_var_count == 0, f"Found {old_var_count} occurrences of old variable ENABLE_AGENTIC_RAG"
    assert new_var_count == 3, f"Expected 3 occurrences of USE_AGENTIC_RAG, found {new_var_count}"
...
def main():
    ...
    try:
        ...
        print("\n📋 Summary of changes:")
        print("- Changed environment variable from ENABLE_AGENTIC_RAG to USE_AGENTIC_RAG")
        print("- Code extraction logic is properly integrated in the scraping pipeline")
        print("- Code will now extract and store code examples when USE_AGENTIC_RAG=true")
