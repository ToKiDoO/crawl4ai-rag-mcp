...
from services.validated_search import (
    ValidationResult,
...
class TestValidatedCodeSearchService:
    ...
    @pytest.mark.asyncio
    async def test_search_and_validate_code_basic(self, validated_search_service, sample_semantic_results):
        """Test basic search and validation workflow."""
        # Mock semantic search
        with patch.object(validated_search_service, '_perform_semantic_search') as mock_semantic:
            mock_semantic.return_value = sample_semantic_results

            # Mock validation
            with patch.object(validated_search_service, '_validate_results_parallel') as mock_validate:
                validated_results = []
    ...
    @pytest.mark.asyncio
    async def test_search_with_confidence_filtering(self, validated_search_service, sample_semantic_results):
    ...
    @pytest.mark.asyncio
    async def test_perform_semantic_search(self, validated_search_service, mock_database_client):
        """Test semantic search functionality."""
        # Mock embedding generation
        with patch('services.validated_search.create_embeddings_batch') as mock_embeddings:
    ...
    @pytest.mark.asyncio
    async def test_validate_results_parallel(self, validated_search_service, sample_semantic_results):
        """Test parallel validation of search results."""
        # Mock Neo4j enabled
        validated_search_service.neo4j_enabled = True
        
        with patch.object(validated_search_service, '_validate_single_result') as mock_validate_single:
    ...
    @pytest.mark.asyncio
    async def test_validate_results_neo4j_disabled(self, validated_search_service, sample_semantic_results):
    ...
    @pytest.mark.asyncio
    async def test_validate_single_result_with_caching(self, validated_search_service, sample_semantic_results):
        ...
        with patch.object(validated_search_service, '_perform_neo4j_validation') as mock_neo4j:
            ...
            validated_result = await validated_search_service._validate_single_result(
                result, include_suggestions=True
            )
    ...
    async def test_perform_neo4j_validation_function(self, validated_search_service):
        ...
        with patch.object(validated_search_service, '_get_neo4j_session') as mock_get_session, \
             patch.object(validated_search_service, '_check_repository_exists') as mock_repo, \
             patch.object(validated_search_service, '_check_function_exists') as mock_func:
    ...
    async def test_perform_neo4j_validation_method(self, validated_search_service):
        ...
        with patch.object(validated_search_service, '_get_neo4j_session') as mock_get_session, \
             patch.object(validated_search_service, '_check_repository_exists') as mock_repo, \
             patch.object(validated_search_service, '_check_method_exists') as mock_method, \
             patch.object(validated_search_service, '_validate_method_signature') as mock_signature:
            ...
            assert validation["is_valid"] is True
            assert len(validation["validation_checks"]) == 3  # repo + method + signature
    ...
    async def test_check_method_exists(self, validated_search_service):
        ...
        exists = await validated_search_service._check_method_exists(
            mock_session, "authenticate", "AuthService", "test-repo"
        )
    ...
    @pytest.mark.asyncio
    async def test_generate_suggestions_for_failed_checks(self, validated_search_service):
    ...
    async def test_get_health_status(self, validated_search_service):
        ...
        with patch('services.validated_search.validate_integration_health') as mock_health:
