...
async def test_smart_crawl_url():
    ...
    async with aiohttp.ClientSession() as session:
        ...
        try:
            async with session.post(f"{url}/v1/tools/list", headers=headers, json=list_tools_payload) as response:
                print(f"List tools response: {response.status}")
                if response.status == 200:
                    data = await response.json()
                    print(f"Available tools: {[tool.get('name', 'unknown') for tool in data.get('tools', [])]}")
                    ...
                    async with session.post(f"{url}/v1/tools/call", headers=headers, json=crawl_payload) as crawl_response:
            ...
            try:
                async with session.post(mcp_url, headers=mcp_headers, json=mcp_payload) as mcp_response:
