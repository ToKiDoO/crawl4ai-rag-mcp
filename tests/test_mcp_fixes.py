...
async def test_scrape_urls():
    ...
    try:
        result = await call_mcp_tool("scrape_urls", {
        ...
        if result.get("success"):
            ...
            if result.get("results") and result["results"][0].get("chunks_stored", 0) > 0:
...
async def test_perform_rag_query():
    ...
    try:
        result = await call_mcp_tool("perform_rag_query", {
            "query": "test query",
            "match_count": 3
        })
...
async def test_smart_crawl_url():
    ...
    try:
        result = await call_mcp_tool("smart_crawl_url", {
            "url": "https://example.com",
            "max_depth": 1
        })
        ...
        return not ("FunctionTool" in str(result))
    except Exception as e:
        print(f"❌ ERROR: {e}")
        return False
