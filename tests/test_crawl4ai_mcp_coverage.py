...
from tests.test_doubles import FakeQdrantClient, FakeEmbeddingService, FakeCrawler


class MockContext:
    """Mock context for testing MCP tools"""
    def __init__(self, crawler=None, database_client=None, reranking_model=None, knowledge_validator=None, repo_extractor=None):
...
@pytest.mark.skipif(os.getenv("USE_KNOWLEDGE_GRAPH", "false") != "true", 
...
class TestKnowledgeGraphTools:
    ...
    async def test_parse_github_repository_success(self):
        ...
        # Test
        result = await parse_github_repository(ctx, repo_url="https://github.com/user/test-repo")
        ...
        mock_extractor.parse_repository.assert_called_once_with("https://github.com/user/test-repo")
...
class TestErrorHandling:
    ...
    async def test_scrape_urls_database_error(self):
        ...
        mock_db_client.add_documents_batch.side_effect = Exception("Database connection failed")
    ...
    async def test_perform_rag_query_no_results(self):
        ...
        from crawl4ai_mcp import perform_rag_query, search_documents
        
        # Patch search_documents to return empty list
        with patch('crawl4ai_mcp.search_documents', return_value=[]):
            # Test
            result = await perform_rag_query(ctx, query="non-existent content", match_count=5)
...
class TestSmartCrawlUrl:
    ...
    async def test_smart_crawl_sitemap(self):
        ...
        with patch('crawl4ai_mcp.crawl_batch') as mock_crawl_batch:
            mock_crawl_batch.return_value = [
                {"url": "https://example.com/page1", "success": True, "markdown": "Page 1", "metadata": {}},
                {"url": "https://example.com/page2", "success": True, "markdown": "Page 2", "metadata": {}}
            ]
            
            from crawl4ai_mcp import smart_crawl_url
            result = await smart_crawl_url(ctx, url="https://example.com/sitemap.xml", max_depth=1)
    ...
    async def test_smart_crawl_llms_txt(self):
        ...
        from crawl4ai_mcp import smart_crawl_url
        result = await smart_crawl_url(ctx, url="https://example.com/llms.txt", return_raw_markdown=True)
...
class TestSearchIntegration:
    ...
    async def test_search_return_raw_markdown(self, mock_crawl, mock_searxng):
        ...
        mock_searxng.return_value = {
            "results": [
                {"url": "https://example.com/1", "title": "Test 1", "content": "Preview 1"}
            ]
        }
        ...
        from crawl4ai_mcp import search
        result = await search(ctx, query="test", return_raw_markdown=True, num_results=1)
...
class TestCodeExamples:
    ...
    async def test_search_code_examples_success(self):
        ...
        with patch.dict(os.environ, {"USE_AGENTIC_RAG": "true"}):
            from crawl4ai_mcp import search_code_examples
            result = await search_code_examples(ctx, query="hello function", match_count=5)
    ...
    async def test_search_code_examples_with_source_filter(self):
        ...
        with patch.dict(os.environ, {"USE_AGENTIC_RAG": "true"}):
            from crawl4ai_mcp import search_code_examples
            result = await search_code_examples(ctx, query="test", source_id="github.com", match_count=5)
