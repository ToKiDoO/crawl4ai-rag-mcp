...
async def test_e2e_search_pipeline():
    ...
    try:
        ...
        if result_data.get("success"):
            ...
            print(f"   ❌ RAG search failed: {result_data.get('error', 'Unknown error')}")
            return False
...
def main():
    ...
    import subprocess
    try:
        result = subprocess.run(['docker', 'ps', '--filter', 'name=searxng-dev', '--format', '{{.Names}}'], 
                               capture_output=True, text=True, check=True)
        if 'searxng-dev' not in result.stdout:
            print("❌ SearXNG service not running. Please start with: make dev-bg-nobuild")
            return False
    except:
