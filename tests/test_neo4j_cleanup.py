...
class TestNeo4jCleanupFunctionality:
    ...
    async def test_successful_repository_cleanup(self, mock_extractor):
        ...
        assert len(mock_extractor.driver.sessions) == 2  # One for validation, one for cleanup
        ...
        for i, pattern in enumerate(expected_patterns):
            ...
            assert pattern in operation_query, f"Expected pattern '{pattern}' not found in query: {operation_query}"
    ...
    async def test_repository_not_found_graceful_handling(self, mock_extractor):
        ...
        with patch.object(mock_extractor.driver, 'session') as mock_session_context:
            mock_session_context.return_value.__aenter__ = AsyncMock(return_value=validation_session)
    ...
    async def test_transaction_rollback_on_failure(self, mock_extractor):
        ...
        with patch.object(mock_extractor.driver, 'session') as mock_session_context:
            mock_session_context.return_value.__aenter__ = AsyncMock(side_effect=[s for s in sessions])
            mock_session_context.return_value.__aexit__ = AsyncMock(return_value=None)
            
            # Expect the cleanup to fail and raise an exception
            with pytest.raises(Exception, match="Repository cleanup failed and was rolled back"):
                await mock_extractor.clear_repository_data(repo_name)
    ...
    async def test_cleanup_operation_order(self, mock_extractor):
        ...
        expected_order = ["methods", "attributes", "functions", "classes", "files", "branches", "commits", "repository"]
        assert operation_types == expected_order, f"Expected {expected_order}, got {operation_types}"
    ...
    async def test_optional_match_usage(self, mock_extractor):
        ...
        for op in operations:
            if "DETACH DELETE" in op["query"] and op["query"] != operations[-1]["query"]:  # Skip final repo deletion
                assert "OPTIONAL MATCH" in op["query"], f"OPTIONAL MATCH not found in query: {op['query']}"
    ...
    async def test_cleanup_statistics_tracking(self, mock_extractor):
        ...
        for op in operations[:-1]:  # All except the final repository deletion
            assert "RETURN count(" in op["query"], f"Missing count return in query: {op['query']}"
            assert "deleted_count" in op["query"], f"Missing deleted_count alias in query: {op['query']}"
...
class TestNeo4jCleanupIntegration:
    ...
    async def test_cleanup_then_reparse_workflow(self, mock_extractor_with_analyzer):
        ...
        with patch.object(mock_extractor_with_analyzer, 'clone_repo', return_value=temp_dir):
            with patch.object(mock_extractor_with_analyzer, 'get_python_files', return_value=[]):
                with patch.object(mock_extractor_with_analyzer, 'get_repository_metadata', return_value={}):
                    with patch.object(mock_extractor_with_analyzer, '_create_graph', return_value=None):
                        with patch('os.path.exists', return_value=False):  # Skip cleanup
                            
                            # First: Parse repository
                            await mock_extractor_with_analyzer.analyze_repository(repo_url, temp_dir)
                            
                            # Then: Cleanup repository  
                            await mock_extractor_with_analyzer.clear_repository_data("test-integration-repo")
        ...
        for session in mock_extractor_with_analyzer.driver.sessions:
            if hasattr(session, 'current_transaction') and session.current_transaction and session.current_transaction.operations:
    ...
    async def test_no_warnings_after_cleanup_reparse(self, mock_extractor_with_analyzer):
                ...
                with patch.object(mock_extractor_with_analyzer, 'get_repository_metadata', return_value={
                    ...
                    with patch.object(mock_extractor_with_analyzer, '_create_graph', return_value=None):
                        with patch('os.path.exists', return_value=False):  # Skip cleanup
                            ...
                            # Cleanup 
                            await mock_extractor_with_analyzer.clear_repository_data("warning-test-repo")
