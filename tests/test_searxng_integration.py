...
from crawl4ai_mcp import search, scrape_urls, perform_rag_query, Crawl4AIContext
...
class TestSearXNGIntegration:
    ...
    async def searxng_health_check(self, searxng_url):
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(f"{searxng_url}/healthz", timeout=aiohttp.ClientTimeout(total=5)) as response:
    ...
    async def test_full_pipeline_search_scrape_store(self, searxng_url, searxng_health_check):
        ...
        with patch.dict(os.environ, {"SEARXNG_URL": searxng_url}):
            ...
            assert search_data.get("success", True)  # Default to True for backward compatibility
            ...
            with patch('crawl4ai_mcp.AsyncWebCrawler') as mock_crawler_class:
                ...
                mock_result.markdown = "# Python Asyncio Tutorial\n\nThis is a test content about asyncio."
