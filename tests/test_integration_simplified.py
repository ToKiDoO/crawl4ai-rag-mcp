...
from tests.test_doubles import FakeCrawler, FakeQdrantClient, FakeEmbeddingService
...
class TestIntegrationSimplified:
    ...
    async def test_scrape_and_store_flow(self, setup_environment, fake_services):
        ...
        chunks = [result.markdown[i:i+100] for i in range(0, len(result.markdown), 100)]
    ...
    async def test_batch_processing_flow(self, setup_environment, fake_services):
        ...
        for url in urls:
            try:
                result = await crawler.arun(url)
                results.append({"url": url, "success": True, "content": result.markdown})
