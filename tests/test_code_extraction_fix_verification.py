...
def test_environment_variable_consistency():
    ...
    print(f"❌ Occurrences of incorrect variable 'ENABLE_AGENTIC_RAG': {incorrect_var_count}")
    
    # Should have exactly 3 occurrences of USE_AGENTIC_RAG and 0 of ENABLE_AGENTIC_RAG
    assert incorrect_var_count == 0, f"Found {incorrect_var_count} occurrences of incorrect variable ENABLE_AGENTIC_RAG"
    assert correct_var_count == 3, f"Expected 3 occurrences of USE_AGENTIC_RAG, found {correct_var_count}"
    ...
    for test_file_path in test_files:
        ...
        if test_file.exists():
            ...
            if incorrect_count > 0:
                raise AssertionError(f"❌ Found {incorrect_count} occurrences of ENABLE_AGENTIC_RAG in {test_file_path}")
            ...
            if 'AGENTIC_RAG' in content:
                print(f"✅ {test_file_path}: {correct_count} USE_AGENTIC_RAG, 0 ENABLE_AGENTIC_RAG")
            else:
...
def main():
    ...
    try:
        ...
        print("\n📋 Summary of the fix:")
        print("- Changed environment variable from ENABLE_AGENTIC_RAG to USE_AGENTIC_RAG in test files")
