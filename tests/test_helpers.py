...
class TestAssertions:
    ...
    def assert_search_result_valid(result: Dict[str, Any]) -> None:
        ...
        for field in required_fields:
            assert field in result, f"Missing required field '{field}' in result: {result}"
        
        assert isinstance(result["score"], (int, float)), f"Score must be numeric, got {type(result['score'])}"
        assert 0 <= result["score"] <= 1, f"Score must be between 0 and 1, got {result['score']}"
        assert isinstance(result["metadata"], dict), f"Metadata must be dict, got {type(result['metadata'])}"
    
    @staticmethod
    def assert_embedding_valid(embedding: List[float], expected_dim: int = 1536) -> None:
        ...
        assert all(isinstance(x, (int, float)) for x in embedding), "All embedding values must be numeric"
    ...
    def assert_api_response_valid(response: Dict[str, Any]) -> None:
        ...
        if response.get("success"):
            ...
            assert isinstance(response["error"], str), f"Error must be string, got {type(response['error'])}"
    
    @staticmethod
    def assert_async_callable(func) -> None:
        """Verify function is async callable."""
        assert asyncio.iscoroutinefunction(func), f"{func.__name__} must be async function"
...
class TestMetrics:
    ...
    def report(self):
        ...
        avg_duration = sum(self.durations.values()) / len(self.durations) if self.durations else 0
        return {
            ...
            "mocks_per_test": self.mocks_created / len(self.durations) if self.durations else 0
        }
