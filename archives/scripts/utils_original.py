    ...
    for retry in range(max_retries):
        try:
            ...
            if retry < max_retries - 1:
                print(
                    f"Error creating batch embeddings (attempt {retry + 1}/{max_retries}, file=sys.stderr): {e}",
                )
                ...
                print(
                    "Attempting to create embeddings individually...", file=sys.stderr,
                )
                embeddings = []
                successful_count = 0
                ...
                print(
                    f"Successfully created {successful_count}/{len(texts, file=sys.stderr)} embeddings individually",
                )
                return embeddings
    ...
    try:
        ...
        for url in unique_urls:
            try:
                ...
                print(
                    f"Error deleting record for URL {url}: {inner_e}", file=sys.stderr,
                )
    ...
    for i in range(0, len(contents), batch_size):
        ...
        if use_contextual_embeddings:
            ...
            if len(contextual_contents) != len(batch_contents):
                print(
                    f"Warning: Expected {len(batch_contents, file=sys.stderr)} results but got {len(contextual_contents)}",
                )
        ...
        for retry in range(max_retries):
            try:
                ...
                if retry < max_retries - 1:
                    print(
                        f"Error inserting batch into Supabase (attempt {retry + 1}/{max_retries}, file=sys.stderr): {e}",
                    )
                    print(f"Retrying in {retry_delay} seconds...", file=sys.stderr)
                    time.sleep(retry_delay)
                    retry_delay *= 2  # Exponential backoff
                    ...
                    print(
                        "Attempting to insert records individually...", file=sys.stderr,
                    )
                    ...
                    if successful_inserts > 0:
                        print(
                            f"Successfully inserted {successful_inserts}/{len(batch_data, file=sys.stderr)} records individually",
                        )
...
def add_code_examples_to_supabase(
    ...
    for url in unique_urls:
        try:
            ...
            print(
                f"Error deleting existing code examples for {url}: {e}", file=sys.stderr,
            )
