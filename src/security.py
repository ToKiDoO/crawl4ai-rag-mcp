...
def validate_api_key(api_key: str | None) -> bool:
    ...
    if any(
        pattern in api_key.lower()
        for pattern in ["<script>", "drop table", "delete from"]
    ):
    ...
    if api_key.startswith("sk-"):
        ...
        if any(pattern in api_key_lower for pattern in forbidden_patterns):
            raise ValueError(
                "Invalid OpenAI API key format: test/mock keys not allowed",
            )
...
def validate_url_security(url: str) -> bool:
    ...
    if any(
        danger in url.lower()
        for danger in ["javascript:", "vbscript:", "data:", "file:", "gopher:", "dict:"]
    ):
...
def handle_error(error: Exception) -> dict[str, Any]:
    ...
    logger.error(f"Error occurred: {type(error).__name__}: {error!s}")
    ...
    if os.getenv("DEBUG", "false").lower() == "true":
        ...
        return {"error": {"message": error_str, "type": type(error).__name__}}
    ...
    return {"error": {"message": message, "type": "error"}}
...
def validate_sse_connection(auth_token: str | None) -> bool:
    """
