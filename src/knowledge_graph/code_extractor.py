...
class CodeExample:
    ...
    method_count: Optional[int] = None  # For classes
    language: str = "python"
...
class Neo4jCodeExtractor:
    ...
    def _generate_class_code(self, class_name: str, methods: List[Dict]) -> str:
        ...
        public_methods = [m for m in methods if m["name"] and not m["name"].startswith("_")]
        if public_methods:
...
async def extract_repository_code(repo_extractor: Any, repo_name: str) -> Dict[str, Any]:
    """
