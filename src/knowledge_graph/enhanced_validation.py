...
class EnhancedScriptAnalyzer:
    ...
    def _analyze_attribute(self, node: ast.Attribute) -> Optional[Dict[str, Any]]:
...
class EnhancedHallucinationDetector:
    ...
    def __init__(self, database_client: Any, neo4j_driver: Any = None):
        ...
        self.validated_search = ValidatedCodeSearchService(database_client, neo4j_driver)
    ...
    async def check_script_hallucinations(
        ...
        try:
            ...
            if not script_file.exists():
                return self._create_error_response(script_path, f"Script file not found: {script_path}")
            
            if not script_file.suffix == ".py":
                return self._create_error_response(script_path, "Only Python (.py) files are supported")
            
            script_content = script_file.read_text(encoding="utf-8")
            
            # Step 1: Analyze script structure
            analysis_result = self.script_analyzer.analyze_script(script_content, script_path)
            
            if "error" in analysis_result:
                return self._create_error_response(script_path, analysis_result["error"])
            ...
            qdrant_validation = await self._perform_qdrant_validation(
                analysis_result, include_code_suggestions
            )
            
            # Step 4: Combine validation results
            combined_validation = self._combine_validation_results(
                neo4j_validation, qdrant_validation
            )
            
            # Step 5: Generate comprehensive report
            report = await self._generate_comprehensive_report(
                script_path, analysis_result, combined_validation, detailed_analysis
            )
            ...
            return self._create_error_response(script_path, f"Detection failed: {str(e)}")
    
    async def _perform_neo4j_validation(self, analysis_result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                import_validations = await self._validate_imports(session, analysis_result.get("imports", []))
                class_validations = await self._validate_classes(session, analysis_result.get("classes", []))
                method_validations = await self._validate_method_calls(session, analysis_result.get("method_calls", []))
                function_validations = await self._validate_function_calls(session, analysis_result.get("functions", []))
            ...
            for element in code_elements:
                ...
                if search_result.get("success") and search_result.get("results"):
                    validation_results.append({
                        ...
                        "validation_summary": search_result.get("validation_summary", {})
                    })
    ...
    def _extract_code_elements_for_search(self, analysis_result: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    async def _validate_imports(self, session, imports: List[Dict]) -> List[Dict[str, Any]]:
            ...
            try:
                if import_info.get("type") == "from_import" and module and name:
                    ...
                    validations.append({
                        ...
                        "found_in_repo": record.get("repo_name") if record else None,
    ...
    async def _validate_classes(self, session, classes: List[Dict]) -> List[Dict[str, Any]]:
    ...
    async def _validate_method_calls(self, session, method_calls: List[Dict]) -> List[Dict[str, Any]]:
    ...
    async def _validate_function_calls(self, session, functions: List[Dict]) -> List[Dict[str, Any]]:
    ...
    def _extract_neo4j_hallucinations(self, neo4j_validation: Dict[str, Any]) -> List[Dict[str, Any]]:
        """Extract hallucinations from Neo4j validation results."""
        hallucinations = []
        
        # Check low confidence validations
        for validation_type in ["import_validations", "class_validations", "method_validations", "function_validations"]:
            for validation in neo4j_validation.get(validation_type, []):
                if validation.get("confidence", 0) < self.MEDIUM_CONFIDENCE_THRESHOLD:
                    hallucinations.append({
                        ...
                        "details": validation,
                        "severity": "high" if validation.get("confidence", 0) < 0.3 else "medium"
                    })
        
        return hallucinations
    
    def _extract_qdrant_hallucinations(self, qdrant_validation: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        for example_group in qdrant_validation.get("code_examples", []):
            ...
            high_confidence_examples = [
                ...
                if ex.get("validation", {}).get("confidence_score", 0) >= self.HIGH_CONFIDENCE_THRESHOLD
            ]
    ...
    def _generate_neo4j_suggestions(self, neo4j_validation: Dict[str, Any]) -> List[str]:
            ...
            for validation in neo4j_validation.get(validation_type, []):
                if not validation.get("exists", False):
                    if validation_type == "import_validations":
                        ...
                        suggestions.append(f"Import '{import_info.get('module', '')}.{import_info.get('name', '')}' not found in knowledge graph. Consider checking the module name or parsing the relevant repository.")
                    elif validation_type == "class_validations":
                        class_info = validation.get("class", {})
                        suggestions.append(f"Class '{class_info.get('name', '')}' not found. Check class name spelling or ensure the relevant repository is parsed.")
    ...
    async def _generate_comprehensive_report(
        ...
        critical_hallucinations = [h for h in hallucinations if h.get("severity") == "high"]
        moderate_hallucinations = [h for h in hallucinations if h.get("severity") == "medium"]
        ...
        report = {
            ...
            "analysis_metadata": {
                ...
                "neo4j_confidence": self._calculate_neo4j_confidence(combined_validation.get("neo4j_validation", {})),
                "qdrant_confidence": self._calculate_qdrant_confidence(combined_validation.get("qdrant_validation", {})),
                "combined_approach": True
            }
        }
    ...
    def _create_error_response(self, script_path: str, error_message: str) -> Dict[str, Any]:
