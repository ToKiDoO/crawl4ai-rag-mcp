...
def validate_crawl_url(url: str) -> dict[str, Any]:
    ...
    if not any(url.startswith(protocol) for protocol in allowed_protocols):
        error_msg = f"URL must start with one of: {', '.join(allowed_protocols)}. Got: '{url}'"
    ...
    if url.startswith(('http://', 'https://')):
        ...
        if url.count('://') > 1:
            return {"valid": False, "error": f"Invalid URL format - multiple protocols found: {url}"}
        ...
        if not domain_part or domain_part.startswith('/'):
            return {"valid": False, "error": f"Invalid URL format - missing domain: {url}"}
