...
async def crawl_batch(
    ...
    for i, url in enumerate(urls):
        logger.debug(f"URL {i+1}/{len(urls)}: {url!r} (type: {type(url).__name__}, length: {len(str(url))})")
        if isinstance(url, str):
            logger.debug(f"  - Stripped: {url.strip()!r}")
            logger.debug(f"  - Contains whitespace: {url != url.strip()}")
            logger.debug(f"  - Starts with http: {url.strip().startswith(('http://', 'https://'))}")
    ...
    if not validation_result["valid"]:
        ...
        if validation_result.get("invalid_urls"):
            logger.error(f"Invalid URLs that were rejected: {validation_result['invalid_urls']}")
        ...
        if validation_result.get("valid_urls"):
            logger.info(f"Valid URLs that passed validation: {validation_result['valid_urls']}")
    ...
    logger.info(f"URL validation successful! {len(validated_urls)} URLs ready for crawling")
    
    # Log info about any URLs that were auto-fixed during validation
    if len(validated_urls) != len(urls):
        logger.info(f"URL count changed during validation: {len(urls)} -> {len(validated_urls)}")
    ...
    logger.info(f"Starting crawl of {len(validated_urls)} validated URLs with max_concurrent={max_concurrent}")
    ...
    try:
        ...
        with SuppressStdout():
            results = await crawler.arun_many(
                urls=validated_urls, config=crawl_config, dispatcher=dispatcher,
            )
        ...
        logger.info(f"Crawling complete: {len(successful_results)} successful, {len(failed_results)} failed")
        
        if successful_results:
            logger.debug(f"Successful URLs: {[r['url'] for r in successful_results]}")
            
        if failed_results:
            logger.warning(f"Failed URLs and reasons:")
            for failed_result in failed_results:
                logger.warning(f"  - {failed_result.url}: success={failed_result.success}, "
        ...
        logger.error(f"Crawler config: cache_mode={crawl_config.cache_mode}, stream={crawl_config.stream}")
        logger.error(f"Dispatcher config: memory_threshold={dispatcher.memory_threshold_percent}%, "
...
async def process_urls_for_mcp(
    ...
    try:
        ...
        if not (hasattr(crawl4ai_ctx, 'crawler') and hasattr(crawl4ai_ctx, 'database_client')):
            return json.dumps({
                "success": False,
