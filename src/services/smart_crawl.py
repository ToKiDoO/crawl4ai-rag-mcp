...
from .crawling import crawl_markdown_file, crawl_recursive_internal_links, process_urls_for_mcp
    ...
    try:
        ...
        if is_sitemap(normalized_url):
            ...
            return await _crawl_sitemap(
                ctx, normalized_url, max_concurrent, chunk_size, return_raw_markdown, query
            )
        elif is_txt(normalized_url):
            logger.info(f"Detected text file: {normalized_url}")
            return await _crawl_text_file(
                ctx, normalized_url, chunk_size, return_raw_markdown
            )
        else:
            logger.info(f"Regular URL, crawling recursively: {normalized_url}")
            return await _crawl_recursive(
                ctx, normalized_url, max_depth, max_concurrent, chunk_size, return_raw_markdown, query
            )
            
    except Exception as e:
        logger.error(f"Error in smart_crawl_url: {e}")
        raise MCPToolError(f"Smart crawl failed: {str(e)}")
...
async def _crawl_sitemap(
    ...
    try:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(url) as response:
                if response.status != 200:
                    raise MCPToolError(f"Failed to fetch sitemap: HTTP {response.status}")
        ...
        if query and not return_raw_markdown and data.get("success"):
            ...
            for q in query:
                try:
                    rag_result = await _perform_rag_query_with_context(ctx, q, source=None, match_count=5)
