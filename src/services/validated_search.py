...
class ValidatedCodeSearchService:
    ...
    async def search_and_validate_code(
        ...
        try:
            ...
            semantic_results = await self._perform_semantic_search(
                query, match_count * 2, source_filter  # Get more results to filter
            )
            ...
            if parallel_validation and self.neo4j_enabled:
                validated_results = await self._validate_results_parallel(
                    semantic_results, include_suggestions
                )
            else:
            ...
            filtered_results = [
                ...
                if result.get("validation", {}).get("confidence_score", 0) >= min_confidence
            ]
            ...
            validation_summary = self._generate_validation_summary(
                semantic_results, validated_results, final_results
            )
        ...
        for result in results:
            try:
                if self.neo4j_enabled:
                    validated_result = await self._validate_single_result(result, include_suggestions)
                else:
    ...
    async def _validate_single_result(
        ...
        cache_key = create_cache_key("validation", result.get("source_id", ""), 
                                   result.get("metadata", {}))
        ...
        if cached_validation:
            ...
            # Perform validation
            validation = await self._perform_neo4j_validation(result, include_suggestions)
    ...
    async def _perform_neo4j_validation(
        ...
        try:
            ...
            repo_exists = await self._check_repository_exists(session, result.get("source_id"))
            ...
            if code_type == "class" and class_name:
                ...
                class_exists = await self._check_class_exists(session, class_name, result.get("source_id"))
                validation_checks.append({
                    "check": "class_exists", 
                    "passed": class_exists,
                    "weight": 0.4
                ...
                if class_exists:
                    structure_valid = await self._validate_class_structure(
                        session, class_name, metadata, result.get("source_id")
                    )
                    validation_checks.append({
                ...
                method_exists = await self._check_method_exists(
                    session, method_name, class_name, result.get("source_id")
                )
                validation_checks.append({
                    "check": "method_exists",
                ...
                if method_exists:
                    signature_valid = await self._validate_method_signature(
                        session, method_name, class_name, metadata, result.get("source_id")
                    )
                    validation_checks.append({
                ...
                function_exists = await self._check_function_exists(
                    session, method_name, result.get("source_id")
                )
                validation_checks.append({
            ...
            suggestions = []
            if include_suggestions and confidence_score < self.HIGH_CONFIDENCE_THRESHOLD:
                suggestions = await self._generate_suggestions(
                    session, result, validation_checks
                )
    ...
    async def _check_class_exists(self, session, class_name: str, source_id: str) -> bool:
        ...
        try:
            ...
            """
            result = await session.run(query, repo_name=source_id, class_name=class_name)
            record = await result.single()
            return record["exists"] if record else False
        except Exception as e:
    ...
    async def _check_method_exists(
        self, session, method_name: str, class_name: str, source_id: str
    ) -> bool:
        ...
        try:
            if class_name:
                ...
                result = await session.run(
                    query, repo_name=source_id, class_name=class_name, method_name=method_name
                )
            else:
                ...
                RETURN count(m) > 0 as exists
                """
                result = await session.run(query, repo_name=source_id, method_name=method_name)
    ...
    async def _check_function_exists(self, session, function_name: str, source_id: str) -> bool:
        ...
        try:
            ...
            """
            result = await session.run(query, repo_name=source_id, function_name=function_name)
            record = await result.single()
            return record["exists"] if record else False
        except Exception as e:
    ...
    async def _validate_class_structure(
        self, session, class_name: str, metadata: Dict, source_id: str
    ) -> bool:
    ...
    async def _validate_method_signature(
        self, session, method_name: str, class_name: str, metadata: Dict, source_id: str
    ) -> bool:
    ...
    async def _generate_suggestions(
        self, session, result: Dict[str, Any], validation_checks: List[Dict]
    ) -> List[str]:
        ...
        for check in validation_checks:
            if not check["passed"]:
                if check["check"] == "repository_exists":
                    suggestions.append(f"Repository '{result.get('source_id')}' not found in knowledge graph. Consider parsing this repository first.")
                elif check["check"] == "class_exists":
                    metadata = result.get("metadata", {})
                    class_name = metadata.get("class_name")
                    suggestions.append(f"Class '{class_name}' not found. Check class name spelling or repository parsing completeness.")
                elif check["check"] == "method_exists":
                    metadata = result.get("metadata", {})
                    method_name = metadata.get("method_name") or metadata.get("name")
                    suggestions.append(f"Method '{method_name}' not found. Verify method name or check if it's inherited.")
                elif check["check"] == "function_exists":
                    metadata = result.get("metadata", {})
                    function_name = metadata.get("method_name") or metadata.get("name")
                    suggestions.append(f"Function '{function_name}' not found. Check function name or module location.")
    ...
    def _generate_validation_summary(
        ...
        high_confidence_count = sum(
            ...
            if result.get("validation", {}).get("confidence_score", 0) >= self.HIGH_CONFIDENCE_THRESHOLD
        )
        ...
        return {
            ...
            "validation_rate": len(validated_results) / len(semantic_results) if semantic_results else 0,
            "high_confidence_rate": high_confidence_count / len(final_results) if final_results else 0,
            "neo4j_available": self.neo4j_enabled,
